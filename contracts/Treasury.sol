...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SRS }
    ...
    address public sRS;
    uint public sRSQueue; // Delays change to sRS address
    ...
    constructor (
        ...
        isReserveToken[ _Rabbit ] = true;
        reserveTokens.push( _Rabbit );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for RS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint RS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( RS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn RS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IRSERC20( RS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sRS ).balanceOf( msg.sender ); // Can only borrow against sRS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with RS
        @param _amount uint
     */
    function repayDebtWithRS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IRSERC20( RS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, RS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( RS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( RS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns RS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match RS decimals
            value_ = _amount.mul( 10 ** IERC20( RS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SRS ) { // 9
            sRSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
