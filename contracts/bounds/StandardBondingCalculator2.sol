...
contract BondingCalculator2 is IBondingCalculator {
    ...
    function markdown(address _pair) external view returns (uint256) {
        ...
        if ( token0 ==RS ||token1 == RS) {
            reserve = token0 == RS ? reserve1 : reserve0;
        } else if(token0 == Anchored || token1 == Anchored){
            reserve = token0 == Anchored ? reserve0 : reserve1;
        }
        return
